...
function install_and_start_node() {
    ...
    echo "正在安装依赖..."
    apt update
    apt install -y build-essential libtool autotools-dev automake pkg-config bsdmainutils python3 libevent-dev libboost-dev libsqlite3-dev || { echo "依赖安装失败"; exit 1; }
    echo "依赖安装完成。"
