...
function l1_install_node() {
	...
	tee /etc/systemd/system/titan-candidate-daemond.service > /dev/null <<EOF
[Unit]
...
EOF
    ...
    systemctl enable titan-candidate-daemond
    systemctl start titan-candidate-daemond
...
function l2_install_node() {
    ...
    docker run --rm -it -v ~/.titanedge:/root/.titanedge nezha123/titan-edge bind --hash=$UUID https://api-test1.container1.titannet.io/api/v2/device/binding
...
function l1_node_log(){
	journalctl -u titan-candidate-daemond -f -o cat
}

# L2节点日志
function l2_node_log(){
	docker logs -f -t priceless_brattain
}

# 停止l1节点
function l1_stop_node(){
	systemctl stop titan-candidate-daemond
	pkill -9 titan-l1-guardian
	echo "L1 节点已停止"
}
...
function l2_stop_node(){
	docker stop priceless_brattain
	echo "L2 节点已停止"
}
...
function validator_install(){
	...
	# 更新和安装必要的软件
    apt update && apt upgrade -y
    apt install -y build-essential git wget jq make gcc nano tmux htop nvme-cli pkg-config libssl-dev libleveldb-dev tar clang bsdmainutils ncdu unzip libleveldb-dev lz4 snapd
    ...
    # 创建服务
    tee /etc/systemd/system/titand.service > /dev/null << EOF
[Unit]
...
function uninstall_node(){
    ...
    case "$response" in
        [yY][eE][sS]|[yY]) 
            ...
            docker rm -f priceless_brattain
